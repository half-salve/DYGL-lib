...
from .message_function import get_message_function
...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, 
    ...
    self.embedding_dimension = self.feat_dim
    ...
    self.time_encoder = TimeEncode(dimension=self.feat_dim)
    self.memory = None
    ...
    self.embedding_module = get_embedding_module(module_type=embedding_module_type,
                                                 ...
                                                 n_node_features=self.feat_dim,
                                                 n_edge_features=self.n_edge_features,
                                                 n_time_features=self.feat_dim,
  ...
  def forward(self, source_nodes, destination_nodes, negative_nodes = None, edge_times=None,
                                  edge_idxs=None, n_neighbors=None):
    """
    Compute temporal embeddings for sources, destinations, and negatively sampled destinations.
    ...
    if negative_nodes is None:
      negative_nodes = destination_nodes
      self.flag = False
  ...
  def set_state(self, state):
    ...
    if state == "train":
        # self.logger.info("set train state")
        self.neighbor_finder = self.train_ngh_finder
        self.embedding_module.neighbor_finder = self.neighbor_finder
        self.train()
    elif state == 'eval' :
        # self.logger.info("set eval state")
