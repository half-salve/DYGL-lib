...
from ..baseEncode import TimeEncode,EmptyEncode,PosEncode
...
class CAWN(torch.nn.Module):
    def __init__(self, ngh_finders,n_feat, e_feat, agg='tree',
        ...
        '''
        --num_neighbors [N_DEGREE [N_DEGREE ...]    a list of neighbor sampling numbers for different hops, when only a single element is input n_layer will be activated
        ...
        --pos_enc {spd,lp,saw}                      way to encode distances, shortest-path distance or landing probabilities, or self-based anonymous walk (baseline)

        --pos_dim POS_DIM                           dimension of the positional embedding
        --pos_sample {multinomial,binary}           two practically different sampling methods that are equivalent in theory

        --walk_pool {attn,sum}                      how to pool the encoded walks, using attention or simple sum, if sum will overwrite all the other walk_ arguments

        --walk_n_head WALK_N_HEAD                   number of heads to use for walk attention
        ...
        --attn_n_head ATTN_N_HEAD                   number of heads used in tree-shaped attention layer, we only use the default here
        
        --time {time,pos,empty}                     how to use time information, we only use the default here

        --drop_out DROP_OUT                         dropout probability for all dropout layers

        --ngh_cache                                 (currently not suggested due to overwhelming memory consumption) cache temporal neighbors previously    calculated to speed up repeated lookup

        --cpu_cores CPU_CORES                       number of cpu_cores used for position encoding
                                                    
        --verbosity VERBOSITY                       verbosity of the program output
        ...
        print(self.num_neighbors,self.num_neighbors)
        self.ngh_finder = None

        # featurese_feat.float()
        self.n_feat_th = torch.nn.Parameter(n_feat.float(), requires_grad=False)
        self.e_feat_th = torch.nn.Parameter(e_feat.float(), requires_grad=False)
    ...
    def forward_embeddings(self, src_idx_l, tgt_idx_l,cut_time_l, subgraphs=None, test=False):
    ...
    def forward(self, src_idx_l, tgt_idx_l , cut_time_l, e_idx_l=None, flag_for_cur_edge=True,test=False):
        ...
        '''
        src_idx_l,tgt_idx_l,cut_time_l,e_idx_l = src_idx_l.numpy(),tgt_idx_l.numpy(),cut_time_l.numpy(),e_idx_l.numpy()
    ...
    def set_state(self, state):
        ...
        if state == "train":
            ...
            self.position_encoder.ngh_finder = self.ngh_finder
            self.train()
        else :
    ...
    def forwardfalse(self, src_idx_l, tgt_idx_l, cut_time_l, subgraphs=None, test=False):
        src_idx_l,tgt_idx_l,cut_time_l= src_idx_l.numpy(),tgt_idx_l.numpy(),cut_time_l.numpy()
        if subgraphs is not None:
            subgraph_src, subgraph_tgt = subgraphs
        else: # not used in our code but is still a useful branch when negative sample is not provided
            subgraph_src = self.grab_subgraph(src_idx_l, cut_time_l, e_idx_l=None)  # TODO: self.grab_subgraph(), with e_idx_l
