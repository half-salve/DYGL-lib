...
import traceback
import abc
from .utils import  get_download_dir, makedirs


'''
本基类DYGLDataset是基于dgl的DGLDataset完成，DGL的数据集基类和是由若干函数组成，我们对其中的函数进行了重写
但是其代码框架保持不变，但是需要更改的函数太多，所以我们没有选择继承，而是直接将代码复制过来
'''

class DYGLDataset(object):
    r"""The basic DYGL dataset for creating graph datasets.
    This class defines a basic template class for DYGL Dataset.
    It is completed based on dgl's DGLDataset https://docs.dgl.ai/generated/dgl.data.DGLDataset.html. 
    Although the framework remains unchanged, there are too many 
    functions that need to be rewritten, so we use transplantation instead of rewriting.
    ...
    def _download(self):
        ...
        makedirs(self.raw_path)#放原始文件的文件夹还未到具体的数据集
        self.download()
